////////////////////////////////////////////////////////////////////////////////
// socket client for producer-consumer with threads                           //
// client can work as producer or consumer based on server's Task? prompt     //
////////////////////////////////////////////////////////////////////////////////

//#include "bits/stdc++.h"
#include <unistd.h>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <fcntl.h>
#include <cstdarg>
#include <poll.h>
#include <sys/socket.h>
#include <sys/param.h>
#include <sys/time.h>
#include <sys/types.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <errno.h>
#include <netdb.h>
#include <pthread.h>
#include <atomic>
#include <vector>
#include <string>
#include <fstream>
#include <iostream>
#include <sstream>
#include <iomanip>

// --- configuration -----------------------------------------------------------
#define MAX_LINE_SIZE           8192    // maximum line size (8KB)

// --- log messages ------------------------------------------------------------
#define LOG_ERROR               0   // errors
#define LOG_INFO                1   // information and notifications
#define LOG_DEBUG               2   // debug messages

int g_debug = LOG_INFO;

void log_msg(int t_log_level, const char *t_form, ...) {
    if (t_log_level && t_log_level > g_debug) return;

    char l_buf[1024];
    va_list l_arg;
    va_start(l_arg, t_form);
    vsnprintf(l_buf, sizeof(l_buf), t_form, l_arg);
    va_end(l_arg);

    switch (t_log_level) {
    case LOG_INFO:
        std::cout << "INF: " << l_buf << std::endl;
        break;
    case LOG_DEBUG:
        std::cout << "DEB: " << l_buf << std::endl;
        break;
    case LOG_ERROR:
        std::cerr << "ERR: (" << errno << "-" << strerror(errno) << ") " 
                  << l_buf << std::endl;
        break;
    }
}

// --- global variables --------------------------------------------------------
int g_socket = -1;
std::atomic<bool> g_running(true);
std::string g_filename;         // filename for file transfer mode

// --- helper functions --------------------------------------------------------
// read line from socket
bool read_line(int t_sock, std::string& t_line) {
    char l_ch;
    t_line.clear();
    while (true) {
        int l_n = read(t_sock, &l_ch, 1);
        if (l_n <= 0) return false;
        if (l_ch == '\n') break;
        t_line += l_ch;
    }
    return true;
}

// send line to socket
bool send_line(int t_sock, const std::string& t_line) {
    std::string l_msg = t_line + "\n";
    int l_len = write(t_sock, l_msg.c_str(), l_msg.length());
    return l_len == (int)l_msg.length();
}

// --- producer thread - sends file lines to server ---------------------------
void* producer_thread(void* t_arg) {
    (void)t_arg;
    
    log_msg(LOG_INFO, "producer thread started - sending file: %s", g_filename.c_str());
    
    // open file for reading
    std::ifstream l_file(g_filename, std::ios::binary);
    if (!l_file.is_open()) {
        log_msg(LOG_ERROR, "unable to open file: %s", g_filename.c_str());
        g_running = false;
        return nullptr;
    }
    
    char l_buffer[MAX_LINE_SIZE];
    int l_line_count = 0;
    
    while (g_running && l_file.getline(l_buffer, sizeof(l_buffer))) {
        std::string l_line = l_buffer;
        
        if (!send_line(g_socket, l_line)) {
            log_msg(LOG_ERROR, "failed to send line to server");
            g_running = false;
            break;
        }
        
        l_line_count++;
        if (l_line_count % 100 == 0) {
            log_msg(LOG_INFO, "sent %d lines", l_line_count);
        }
        
        // wait for OK response
        std::string l_response;
        if (!read_line(g_socket, l_response)) {
            log_msg(LOG_ERROR, "failed to read response from server");
            g_running = false;
            break;
        }
        
        if (l_response != "OK") {
            log_msg(LOG_INFO, "server response: %s", l_response.c_str());
        }
    }
    
    l_file.close();
    
    // send end-of-file marker
    send_line(g_socket, "quit");
    
    log_msg(LOG_INFO, "producer thread exiting - sent %d lines", l_line_count);
    g_running = false;
    return nullptr;
}

// --- consumer thread - receives lines from server and saves to file ---------
void* consumer_thread(void* t_arg) {
    (void)t_arg;
    
    log_msg(LOG_INFO, "consumer thread started - saving to file: %s", g_filename.c_str());
    
    // open file for writing
    std::ofstream l_file(g_filename, std::ios::binary);
    if (!l_file.is_open()) {
        log_msg(LOG_ERROR, "unable to create file: %s", g_filename.c_str());
        g_running = false;
        return nullptr;
    }
    
    int l_line_count = 0;
    
    while (g_running) {
        // read line from server
        std::string l_line;
        if (!read_line(g_socket, l_line)) {
            log_msg(LOG_ERROR, "failed to read from server");
            g_running = false;
            break;
        }
        
        // write line to file
        l_file << l_line << "\n";
        l_line_count++;
        
        if (l_line_count % 100 == 0) {
            log_msg(LOG_INFO, "received %d lines", l_line_count);
        }
        
        // send OK response
        if (!send_line(g_socket, "OK")) {
            log_msg(LOG_ERROR, "failed to send OK to server");
            g_running = false;
            break;
        }
    }
    
    l_file.close();
    log_msg(LOG_INFO, "consumer thread exiting - received %d lines", l_line_count);
    return nullptr;
}

// --- help --------------------------------------------------------------------
void help(int t_narg, char **t_args) {
    if (t_narg <= 1 || !strcmp(t_args[1], "-h")) {
        std::cout << "\n"
                  << "  socket client for file transfer\n"
                  << "\n"
                  << "  usage: " << t_args[0] << " [-h -d] ip_or_name port_number filename\n"
                  << "\n"
                  << "    -d        debug mode \n"
                  << "    -h        this help\n"
                  << "    filename  file to send (producer) or receive (consumer)\n"
                  << "\n"
                  << "  if file exists:     client becomes PRODUCER and sends file\n"
                  << "  if file not exists: client becomes CONSUMER and saves received data\n"
                  << "\n";

        exit(EXIT_SUCCESS);
    }
}

// --- main --------------------------------------------------------------------
int main(int t_narg, char **t_args) {
    if (t_narg <= 3) help(t_narg, t_args);

    int l_port = 0;
    char *l_host = nullptr;
    char *l_filename = nullptr;

    // parse arguments
    for (int i = 1; i < t_narg; i++) {
        if (!strcmp(t_args[i], "-d"))
            g_debug = LOG_DEBUG;
        else if (!strcmp(t_args[i], "-h"))
            help(t_narg, t_args);
        else if (*t_args[i] != '-') {
            if (!l_host)
                l_host = t_args[i];
            else if (!l_port)
                l_port = atoi(t_args[i]);
            else if (!l_filename)
                l_filename = t_args[i];
        }
    }

    if (!l_host || !l_port || !l_filename) {
        log_msg(LOG_INFO, "host, port or filename is missing!");
        help(t_narg, t_args);
        exit(EXIT_FAILURE);
    }

    g_filename = l_filename;

    // determine role based on file existence
    bool l_file_exists = (access(l_filename, F_OK) == 0);
    const char* l_role = l_file_exists ? "producer" : "consumer";
    
    log_msg(LOG_INFO, "file '%s' %s - becoming %s", 
            l_filename, 
            l_file_exists ? "exists" : "does not exist",
            l_role);

    log_msg(LOG_INFO, "connecting to '%s':%d.", l_host, l_port);

    // resolve hostname
    addrinfo l_ai_req, *l_ai_ans;
    bzero(&l_ai_req, sizeof(l_ai_req));
    l_ai_req.ai_family = AF_INET;
    l_ai_req.ai_socktype = SOCK_STREAM;

    int l_get_ai = getaddrinfo(l_host, nullptr, &l_ai_req, &l_ai_ans);
    if (l_get_ai) {
        log_msg(LOG_ERROR, "unknown host name!");
        exit(EXIT_FAILURE);
    }

    sockaddr_in l_cl_addr = *(sockaddr_in *)l_ai_ans->ai_addr;
    l_cl_addr.sin_port = htons(l_port);
    freeaddrinfo(l_ai_ans);

    // create socket
    g_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (g_socket == -1) {
        log_msg(LOG_ERROR, "unable to create socket");
        exit(EXIT_FAILURE);
    }

    // connect to server
    if (connect(g_socket, (sockaddr *)&l_cl_addr, sizeof(l_cl_addr)) < 0) {
        log_msg(LOG_ERROR, "unable to connect to server");
        exit(EXIT_FAILURE);
    }

    log_msg(LOG_INFO, "connected to server");

    // read Task? prompt from server
    std::string l_task_prompt;
    if (!read_line(g_socket, l_task_prompt)) {
        log_msg(LOG_ERROR, "failed to read task prompt from server");
        close(g_socket);
        exit(EXIT_FAILURE);
    }

    log_msg(LOG_INFO, "server asks: %s", l_task_prompt.c_str());

    // Send role to server (automatically determined)
    if (!send_line(g_socket, l_role)) {
        log_msg(LOG_ERROR, "failed to send role to server");
        close(g_socket);
        exit(EXIT_FAILURE);
    }

    log_msg(LOG_INFO, "role selected: %s", l_role);

    pthread_t l_worker_thread;

    // start appropriate thread based on role
    if (!strcmp(l_role, "producer")) {
        // start producer thread
        pthread_create(&l_worker_thread, nullptr, producer_thread, nullptr);
        
        // wait for thread
        pthread_join(l_worker_thread, nullptr);
        
    } else if (!strcmp(l_role, "consumer")) {
        // start consumer thread
        pthread_create(&l_worker_thread, nullptr, consumer_thread, nullptr);
        
        // wait for thread
        pthread_join(l_worker_thread, nullptr);
        
    } else {
        log_msg(LOG_ERROR, "invalid role: %s", l_role);
        close(g_socket);
        exit(EXIT_FAILURE);
    }

    // cleanup
    close(g_socket);
    log_msg(LOG_INFO, "client finished");

    return EXIT_SUCCESS;
}
